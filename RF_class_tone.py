...
# Random Forest Classification on tone
# Trains and evaluates a RF classifier on tones from
# time-binned spike counts of neurons
# Input: Matrices of binned spike counts, tone labels
# Outputs: Writes a ".mat" of test, validaiton, and training scores on the input set


# Set name of data set to load here
curr_data_set = '03_30_I_Blocks231_comps_data.mat'
out_file = '03_30_I_Blocks231_RF_tone_preds.mat'
...
def load_data_set():
    ...
    #Stmtx = mat['STbintrim']
    Stmtx = mat['STgrestone_sf']
    
    # For mvmt residual, no transpose required, matrix is 3d
    #Stmtx = Stmtx[:,:,8]
    # For grid residual, no transpose required
...
estimator  = RandomForestClassifier(n_estimators=1000, max_features='sqrt', oob_score='True', class_weight='balanced')

# Run and Calculate Score
# Modify here to pull output for prediction checking
#score = cross_val_score(estimator, X, Y)

# Optional: split as a time series
